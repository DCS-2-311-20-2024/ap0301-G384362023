...
export function makeCBRobot() {
  ...
  const body = new THREE.Group;
  body.add(new THREE.Mesh(bodyGeometry, cardboardMaterial));
  ...
  armL.position.set(bodyW/3 + armW, legLen + bodyH - armLen/2, 0);
  cardboardRobot.add(armL);

  const armR = new THREE.Mesh(armGeometry, cardboardMaterial);
  armR.position.set(-bodyW/3 - armW, legLen + bodyH - armLen/2, 0);
